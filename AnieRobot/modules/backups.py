...
import BgtRobot.modules.sql.notes_sql as sql
from BgtRobot import dispatcher, LOGGER, OWNER_ID, SUDO_USERS, MESSAGE_DUMP
from BgtRobot.__main__ import DATA_IMPORT
from BgtRobot.modules.helper_funcs.chat_status import user_admin
from BgtRobot.modules.helper_funcs.misc import build_keyboard, revert_buttons
from BgtRobot.modules.helper_funcs.msg_types import get_note_type
from BgtRobot.modules.rules import get_rules
import BgtRobot.modules.sql.rules_sql as rulessql
from BgtRobot.modules.sql import warns_sql as warnssql
import BgtRobot.modules.sql.blacklist_sql as blacklistsql
from BgtRobot.modules.sql import disable_sql as disabledsql
from BgtRobot.modules.sql import cust_filters_sql as filtersql
import BgtRobot.modules.sql.welcome_sql as welcsql
import BgtRobot.modules.sql.locks_sql as locksql
from BgtRobot.modules.connection import connected
...
def import_data(update, context):
	...
	if msg.reply_to_message and msg.reply_to_message.document:
		...
		try:
			...
			return msg.reply_text("There is problem while importing the data! Please ask in @Bgt_chat about why this happened.")
		# Check if backup is from self
		try:
			if str(bot.id) != str(data[str(chat.id)]['bot']):
				return msg.reply_text("Restoring a backup from another bot is not suggested and might cause problems. Documents, photos, videos, audios, records might not be restored as they should have been. However, You can still request a feature regarding this in @AnjaanDuniya2!")
		except:
			pass
		# Select data source
		...
		try:
			...
			msg.reply_text("An error occurred while recovering the data. The process failed. If you experience a problem with this, please ask in @AdityaDiscus My owner and the community will be happy to help. Also, bug report makes me even better!\nThank you!")
...
This module is still in beta! Report bugs in @Bgt_Chat!
"""
